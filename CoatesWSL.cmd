...
SET DEFEXL=NONO& SET /p DEFEXL=[only for troubleshooting, not recommended] Type X to exclude from Windows Defender:
...
REM ## Download Ubuntu
IF NOT EXIST "%TEMP%\Ubuntu2204.tar.gz" POWERSHELL.EXE -Command "Start-BitsTransfer -source https://cloud-images.ubuntu.com/wsl/jammy/current/ubuntu-jammy-wsl-amd64-wsl.rootfs.tar.gz -destination '%TEMP%\Ubuntu2204.tar.gz'"
%DISTROFULL:~0,1%: & MKDIR "%DISTROFULL%" & CD "%DISTROFULL%" & MKDIR logs > NUL
(ECHO [WSL Inputs] && ECHO. && ECHO.   Distro: %DISTRO% && ECHO.     Path: %DISTROFULL% && ECHO. RDP Port: %RDPPRT% && ECHO. SSH Port: %SSHPRT% && ECHO.DPI Scale: %WINDPI% && ECHO.) > ".\logs\%TIME:~0,2%%TIME:~3,2%%TIME:~6,2% WSL Inputs.log"


REM ## https://github.com/DDoSolitary/LxRunOffline if updated needed
...
REM ## create uninstall script for wsl instance
ECHO:
...
REM ## Installing wsl distro
ECHO Installing WSL Distro [%DISTRO%] to "%DISTROFULL%" & ECHO This will take a few minutes, please wait... 
...
ECHO:& ECHO Installing Ubuntu 22.04
...
ECHO Apt update and clone repo packages
...
%GO% "rm -rf /etc/apt/apt.conf.d/20snapd.conf /etc/systemd/system/snap* /var/cache/snapd /etc/rc2.d/S01whoopsie /etc/init.d/console-setup.sh" REM ## ; "echo 'echo 1' > /usr/sbin/runlevel"
...
%GO% "mv /tmp/CoatesWSL/dist/etc/dpkg/dpkg.cfg.d/01_nodoc /etc/dpkg/dpkg.cfg.d ; dpkg -i /tmp/CoatesWSL/deb/python*.deb /tmp/CoatesWSL/deb/gzip_1.10-4ubuntu1_amd64.deb /tmp/CoatesWSL/deb/aria2_1.36.0-1_amd64.deb /tmp/CoatesWSL/deb/libaria2-0_1.36.0-1_amd64.deb /tmp/CoatesWSL/deb/libc-ares2_1.18.1-1build1_amd64.deb /tmp/CoatesWSL/deb/libssh2-1_1.10.0-3_amd64.deb ; apt-mark hold gzip ; pip3 install apt-select ; chmod +x /tmp/CoatesWSL/dist/usr/local/bin/apt-fast ; cp -p /tmp/CoatesWSL/dist/usr/local/bin/apt-fast /usr/local/bin" > ".\logs\%TIME:~0,2%%TIME:~3,2%%TIME:~6,2% Configure apt-fast Downloader.log" 2>&1

REM ## remove troublesome and uneeded packages
...
REM ## loop to set local repo based off country
:APTRELY
...
REM ## configure xfce for different dpi settings
SET /A SESMAN = %RDPPRT% - 50
%GO% "which schtasks.exe" > "%TEMP%\SCHT.tmp" & set /p SCHT=<"%TEMP%\SCHT.tmp"
%GO% "sed -i 's#SCHT#%SCHT%#g' /tmp/CoatesWSL/dist/usr/local/bin/restartwsl ; sed -i 's#DISTRO#%DISTRO%#g' /tmp/CoatesWSL/dist/usr/local/bin/restartwsl"
IF %LINDPI% GEQ 192 ( %GO% "sed -i 's/HISCALE/2/g' /tmp/CoatesWSL/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml" )
IF %LINDPI% GEQ 288 ( %GO% "sed -i 's/HISCALE/3/g' /tmp/CoatesWSL/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml" )
IF %LINDPI% GEQ 192 ( %GO% "sed -i 's/Default-hdpi/Default-xhdpi/g' /tmp/CoatesWSL/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xfwm4.xml" )
IF %LINDPI% GEQ 192 ( %GO% "sed -i 's/16/32/g' /tmp/CoatesWSL/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xfwm4.xml" )
IF %LINDPI% GEQ 192 ( %GO% "sed -i 's/QQQ/96/g' /tmp/CoatesWSL/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml" )
IF %LINDPI% LSS 192 ( %GO% "sed -i 's/QQQ/%LINDPI%/g' /tmp/CoatesWSL/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml" )
IF %LINDPI% LSS 192 ( %GO% "sed -i 's/HISCALE/1/g' /tmp/CoatesWSL/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml" )
IF %LINDPI% LSS 120 ( %GO% "sed -i 's/Default-hdpi/Default/g' /tmp/CoatesWSL/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xfwm4.xml" )


REM ## configure rdp remote access
%GO% "mkdir -p /dev/dri ; mknod -m 666 /dev/dri/card0 c 226 0 ; mknod -m 666 /dev/dri/renderD128 c 226 128 ; mknod -m 666 /dev/fb0 c 29 0"
%GO% "sed -i 's/ CoatesWSL/ %DISTRO%/g' /tmp/CoatesWSL/dist/etc/skel/.config/xfce4/panel/whiskermenu-1.rc"
%GO% "sed -i 's/ListenPort=3350/ListenPort=%SESMAN%/g' /etc/xrdp/sesman.ini"
%GO% "sed -i 's/thinclient_drives/.CoatesWSL/g' /etc/xrdp/sesman.ini"
%GO% "sed -i 's/port=3389/port=%RDPPRT%/g' /tmp/CoatesWSL/dist/etc/xrdp/xrdp.ini ; sed -i 's/\\h/%DISTRO%/g' /tmp/CoatesWSL/dist/etc/skel/.bashrc"
...
%GO% "sed -i 's/WSLINSTANCENAME/%DISTRO%/g' /tmp/CoatesWSL/dist/usr/local/bin/initwsl"
...
%GO% "chmod 644 /tmp/CoatesWSL/dist/etc/wsl.conf ; chmod 644 /tmp/CoatesWSL/dist/var/lib/xrdp-pulseaudio-installer/*.so"
%GO% "chmod 755 /tmp/CoatesWSL/dist/etc/profile.d/CoatesWSL.sh ; chmod 755 /tmp/CoatesWSL/dist/usr/local/bin/restartwsl ; chmod 755 /tmp/CoatesWSL/dist/usr/local/bin/initwsl ; chmod -R 700 /tmp/CoatesWSL/dist/etc/skel/.config ; chmod -R 7700 /tmp/CoatesWSL/dist/etc/skel/.local ; chmod 700 /tmp/CoatesWSL/dist/etc/skel/.mozilla ; chmod +x /tmp/CoatesWSL/dist/etc/skel/Desktop/Falkon.desktop ; chmod +x /tmp/CoatesWSL/dist/etc/skel/Desktop/Seamonkey.desktop"
...
%GO% "cp -Rp /tmp/CoatesWSL/dist/* / ; cp -Rp /tmp/CoatesWSL/dist/etc/skel/.config /root ; cp -Rp /tmp/CoatesWSL/dist/etc/skel/.local /root ; chown -R xrdp:root /etc/xrdp"


REM ## user creation and final config before spin up
...
%GO% "sed -i 's/PLACEHOLDER/%XU%/g' /tmp/CoatesWSL/CoatesWSL.rdp"
%GO% "sed -i 's/CoatesHostname/%COMPUTERNAME%/g' /tmp/CoatesWSL/CoatesWSL.rdp"
%GO% "sed -i 's/RDPPRT/%RDPPRT%/g' /tmp/CoatesWSL/CoatesWSL.rdp"
%GO% "cp /tmp/CoatesWSL/CoatesWSL.rdp ./CoatesWSL._"
...
TYPE .tmp>.tmpsec.txt
COPY /y /b CoatesWSL._+.tmpsec.txt "%DISTROFULL%\%DISTRO% (%XU%) Desktop.rdp" > NUL
DEL /Q CoatesWSL._ .tmp*.* > NUL


REM ## ssh creation for https://git.coates.io/-/profile/keys
REM ## START /MIN "Creating SSH Keys..." %GO% cd /tmp ; "ssh-keygen -t rsa -f "/tmp/id_rsa" -N """ ; "mv /tmp/id* ~/.ssh/" ; cat ~/.ssh/id_rsa.pub
REM ## ECHO. & ECHO "Please copy the following key and paste it in https://git.coates.io/-/profile/keys then press enter to continue cloning and installing sb-toolkit." & PAUSE & GOTO SB)
REM ## :SB
REM ## START /MIN "Cloning & Installing sb-toolkit..." %GO% "mkdir -p ~/dev" ; cd ~/dev ; "git clone git@git.coates.io:all/sb-toolkit.git" ; echo 'source ~/dev/sb-toolkit/load.sh' >> ~/.bashrc ; echo 'source ~/.bashrc' >> ~/.bash_profile ; source ~/.bash_profile


REM ## opening ports for remote goodness
ECHO:
...
POWERSHELL -C "$WAI = (whoami) ; (Get-Content .\rootfs\tmp\CoatesWSL\CoatesWSL.xml).replace('AAAA', $WAI) | Set-Content .\rootfs\tmp\CoatesWSL\CoatesWSL.xml"
POWERSHELL -C "$WAC = (pwd)    ; (Get-Content .\rootfs\tmp\CoatesWSL\CoatesWSL.xml).replace('QQQQ', $WAC) | Set-Content .\rootfs\tmp\CoatesWSL\CoatesWSL.xml"
SCHTASKS /Create /TN:%DISTRO% /XML .\rootfs\tmp\CoatesWSL\CoatesWSL.xml /F
ECHO:
ECHO:      Start: %RUNSTART%
ECHO:        End: %RUNEND%
%GO%  "echo -ne '   Packages:'\   ; dpkg-query -l | grep "^ii" | wc -l "
ECHO: 
ECHO:  - xRDP Server listening on port %RDPPRT% REM ## and SSHd on port %SSHPRT%.
ECHO: 
